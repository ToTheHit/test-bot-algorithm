...
import { PointModel, PortModel } from '@projectstorm/react-diagrams';
import {
  snap,
  samePosition,
  sameX,
    ...
    this.registerAction(
      new Action({
        ...
        fire: event => {
          ...
          if (
            this.element instanceof PointModel
            // !this.element.getLink().getTargetPort()
          ) {
            this.sourceBefore = {
              id: this.element.getLink().getID(),
              points: this.element.getLink().getPoints(),
              targetEntity: {
                node: this.element.getLink().getTargetPort()?.getParent().getID(),
                port: this.element.getLink().getTargetPort()?.getID()
              }
            };
          }
        }
      ...
      new Action({
        ...
        fire: event => {
          ...
          if (
            ...
            const targetModel = this.engine.getMouseElement(event.event);
            ...
            if (targetModel instanceof PortModel) {
              ...
              if (sourcePort.canLinkToPort(targetModel)) {
                ...
                const sourceAfter = {
                  id: this.element.getLink().options.id,
                  points: this.element.getLink().getPoints(),
                  targetEntity: {
                    node: this.element.getLink().getTargetPort()?.getParent().getID(),
                    port: this.element.getLink().getTargetPort()?.getID()
    ...
    return this.engine
      ...
      .map(link => ({
        ...
        points: link.getPoints()
          .reduce((acc, point) => {
            acc[point.getID()] = { position: point.getPosition() };
